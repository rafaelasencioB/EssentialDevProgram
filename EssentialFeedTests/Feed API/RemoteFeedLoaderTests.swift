...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_deliversItemsOn200HTTPResponseWithJSONItems() {
        ...
        let item1 = makeItem(id: UUID(), imageURL: URL(string: "any-URL.com")!)
        
        let item2 = makeItem(id: UUID(), description: "a description", location: "a location", imageURL: URL(string: "any-URL.com")!)
    ...
    private func makeItem(id: UUID, description: String? = nil,
        ...
        ].reduce(into: [String: Any]()) { (accuDict, element) in
            if let value = element.value {
                accuDict[element.key] = value
            }
        }
        return (item, json)
    }
    ...
    private func expect(_ sut: RemoteFeedLoader,
        ...
        sut.load { receivedResult in
            ...
            case let (.success(receivedItems), .success(expectedItems)):
                XCTAssertEqual(receivedItems, expectedItems, file: file,
                               line: line)
            case let (.failure(receivedError as RemoteFeedLoader.Error), .failure(expectedError as RemoteFeedLoader.Error)):
                XCTAssertEqual(receivedError, expectedError, file: file,
                               line: line)
            default:
